...
void        msdp_send_update              ( SOCKET_DATA *d );
void        msdp_update_var               ( SOCKET_DATA *d, char *var, char *fmt, ... );
void        msdp_update_var_instant       ( SOCKET_DATA *d, char *var, char *fmt, ... );

void        msdp_configure_arachnos       ( SOCKET_DATA *d, int index );
void        msdp_configure_pluginid       ( SOCKET_DATA *d, int index );

void        write_msdp_to_descriptor      ( SOCKET_DATA *d, char *src, int length );
...
void init_msdp_table(void)
{
	...
	for (index = 0 ; *msdp_table[index].name ; index++)
	{
		if (strcmp(msdp_table[index].name, msdp_table[index+1].name) > 0)
		{
			...
			{
				log_string("\e[31minit_msdp_table: Improperly sorted variable: %s.\e0m", msdp_table[index+1].name);
			}
		}
	}
...
void msdp_update_var(SOCKET_DATA *d, char *var, char *fmt, ...)
{
	...
	if (index == -1)
	{
		log_string("msdp_update_var: Unknown variable: %s.", var);
...
void msdp_update_var_instant(SOCKET_DATA *d, char *var, char *fmt, ...)
{
	...
	if (index == -1)
	{
		log_string("msdp_update_var_instant: Unknown variable: %s.", var);
...
char *msdp_get_var(SOCKET_DATA *d, char *var)
{
	int index;
	...
	if (index == -1)
	{
		log_string("msdp_get_var: Unknown variable: %s.", var);
...
void process_msdp_index_val(SOCKET_DATA *d, int var_index, char *val )
{
	int val_index;

	val_index = msdp_find(val);

//	log_string("process_msdp_index_val(%d, %d, %s)", var_index, val_index, val);
...
void process_msdp_array( SOCKET_DATA *d, int var_index, char *val)
{
	char buf[MAX_OUTPUT], *pto, *pti;
...
void process_msdp_varval( SOCKET_DATA *d, char *var, char *val )
{
	...
	if (HAS_BIT(msdp_table[var_index].flags, MSDP_FLAG_COMMAND))
	{
		...
		{
//			log_string("msdp_varval: %s array %s.", var, val);
...
void msdp_command_list(SOCKET_DATA *d, int index)
{
	char *ptr, buf[MAX_OUTPUT];
	int flag;
...
void msdp_command_report(SOCKET_DATA *d, int index)
{
...
void msdp_command_reset(SOCKET_DATA *d, int index)
{
	int flag;
...
void msdp_command_send(SOCKET_DATA *d, int index)
{
...
void msdp_command_unreport(SOCKET_DATA *d, int index)
{
	if (!HAS_BIT(msdp_table[index].flags, MSDP_FLAG_REPORTABLE))
	{
		return;
...
void write_msdp_to_descriptor(SOCKET_DATA *d, char *src, int length)
{
	...
	{
		text_to_socket(d, src, length);
	}
	else
	{
		length = msdp2json((unsigned char *) src, length, out);

		text_to_socket(d, out, length);
	}
}
