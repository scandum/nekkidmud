...
void        debug_telopts            ( SOCKET_DATA *d, unsigned char *src, int srclen );

int         process_do_eor           ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_will_ttype       ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_ttype_is      ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_naws          ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_will_new_environ ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_new_environ   ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_do_charset       ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_charset       ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_do_mssp          ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_do_msdp          ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_msdp          ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_do_gmcp          ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_gmcp          ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_do_mccp2         ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_dont_mccp2       ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         skip_sb                  ( SOCKET_DATA *d, unsigned char *src, int srclen );
void        descriptor_printf        ( SOCKET_DATA *d, char *fmt, ...);
int         start_mccp2              ( SOCKET_DATA *d );
void        process_mccp2            ( SOCKET_DATA *d );
int         process_do_mccp3         ( SOCKET_DATA *d, unsigned char *src, int srclen );
int         process_sb_mccp3         ( SOCKET_DATA *d, unsigned char *src, int srclen );
void        send_ga                  ( SOCKET_DATA *d );
void        send_eor                 ( SOCKET_DATA *d );

struct telopt_type
{
	int      size;
	unsigned char   * code;
	int   (* func) (SOCKET_DATA *d, unsigned char *src, int srclen);
};
...
void unannounce_support( SOCKET_DATA *d )
{
	int i;
		...
		for (cnt = 0 ; cnt < skip ; cnt++)
		{
			switch (out[cnt])
			{
				...
				default:
					...
					{
						text_to_socket(d, (char *) out + cnt, 1);
					}
					break;
			}
		}
...
void process_mccp2( SOCKET_DATA *d )
{
	int length = mud->mccp_len - d->mth->mccp2->avail_out;

	if (write(d->control, mud->mccp_buf, length) < 1)
	{
